...
function runIsolatedHighResetSimulation(odds, profitPercent) {
...
console.log("RESEARCH: ISOLATED CLUSTER + HIGH RESET TRIGGERS");
[0.43, 1.0, 1.5, 2.0].forEach(p => {
    const res = runIsolatedHighResetSimulation(11.5, p);
    console.log(`\nThreshold: Profit +${(p * 100).toFixed(0)}%`);
