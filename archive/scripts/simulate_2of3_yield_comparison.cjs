...
function runYieldComparison(yieldThreshold) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (drawRevenue > 0 && activeChases.length > 0) {
            // Logic: Reset only if profit covers X% of the money spent on active seeds
...
console.log("RESEARCH: YIELD THRESHOLD COMPARISON");
console.log("Odd: 5.7 | Sequence: Double-Stay 27-Step");

[0.5, 0.75, 1.0].forEach(t => {
    const res = runYieldComparison(t);
    console.log(`\nThreshold: ${(t * 100)}% Yield`);
