...
function Grid({ col, data, numbToColor = [], length }) {
    ...
    return (
        <div className={`grid gap-[2px] bg-green-950 rounded-sm text-black p-2`} style={{ gridTemplateColumns: `repeat(${col}, 1fr)` }}>
            {arr.map((n, i) => (
                <div key={i} className={`aspect-square rounded-sm text-[12px] font-bold p-1 border grid items-center border-green-800/50 justify-center transition-all duration-300 ${!numbToColor.includes(n) && data.includes(n)
